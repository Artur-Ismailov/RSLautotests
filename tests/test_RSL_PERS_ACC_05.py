...
def test_RSL_PERS_ACC_05(page: Page):
    ...
    with allure.step("2. Кликнуть на ссылку «Забыли пароль?»"):
        ...
        expect(login_page.get_reset_instruction_text()).to_contain_text(
    ...
    with allure.step("3. В поле ввода «Адрес электронной почты» ввести «test@invalid»"):
        ...
        expect(login_page.get_reset_email_input()).to_have_value("test@invalid", timeout=5000)

    with allure.step("4. Кликнуть на кнопку [Получить пароль]"):
        login_page.click_reset_submit_button()

        expect(login_page.get_email_error()).to_be_visible(timeout=5000)
        expect(login_page.get_email_error()).to_have_text("Ошибка! Неверный формат", timeout=5000)