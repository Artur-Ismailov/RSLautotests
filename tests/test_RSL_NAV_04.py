...
def test_RSL_NAV_04(page: Page):
    ...
    with allure.step("2.В выпадающем списке кликнуть «График работы»"):
        ...
        expect(work_schedule_page.get_breadcrumbs_container()).to_be_visible(timeout=5000)
        expect(work_schedule_page.get_breadcrumb_home_link()).to_be_visible(timeout=5000)
        expect(work_schedule_page.get_breadcrumb_readers_link()).to_be_visible(timeout=5000)
    ...
    with allure.step("3. В хлебных крошках кликнуть «Читателям»"):
        ...
        expect(readers_page.get_breadcrumb_home_link()).to_be_visible(timeout=5000)