...
def test_RSL_PERS_ACC_04(page: Page):
    ...
    with allure.step("3. В поле ввода «Адрес электронной почты» ввести «qatester12348765@mail.ru»"):
        ...
        expect(login_page.get_reset_email_input()).to_have_value("qatester12348765@mail.ru", timeout=5000)

    with allure.step("4. Кликнуть на кнопку [Получить пароль]"):
        login_page.click_reset_submit_button()

        expect(login_page.get_success_message()).to_be_visible(timeout=10000)
        expect(login_page.get_success_message()).to_contain_text("В ближайшее время Вы должны получить письмо с дальнейшими инструкциями.", timeout=5000)