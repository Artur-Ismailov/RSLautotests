...
def test_RSL_PERS_ACC_01(page: Page):
    ...
    with allure.step("2. В поле ввода «Адрес электронной почты или номер читательского билета» ввести «invalid@mail.ru»"):
        ...
        expect(login_page.get_username_input()).to_have_value("invalid@mail.ru", timeout=5000)

    with allure.step("3. В поле ввода «Пароль» ввести «password»"):
        login_page.fill_password_input("password")

        expect(login_page.get_password_input()).to_have_value("password", timeout=5000)

    with allure.step("4. Кликнуть на кнопку [Войти]"):
        login_page.click_login_button()

        expect(login_page.get_error_message()).to_be_visible(timeout=10000)
        expect(login_page.get_error_message()).to_contain_text("Неправильное имя пользователя или пароль", timeout=5000)
        expect(login_page.get_error_message()).to_contain_text("Или данные были удалены согласно 152-ФЗ", timeout=5000)